    ...
    public static void doTermScoring(){
        ...
        List<ScoreFactor> terms = data.getAllTermsForScoring();
            for(ScoreFactor e : terms){
                //double idf = calculateIdf(totalDoc, e.getCount())/100;
                double idf = calculateIdf(6043, e.getCount());
     ...
     public static void doPRScoring(){
        try{
            ...
            long t = data.getTotalDocs();
            for(ScoreFactor e : terms){
                //double idf = calculateIdf(totalDoc, e.getCount());
                //double idf =   e.getCount()/(double) t ;
                //double idf = 10 *((e.getRank() *e.getCount()) /(double) t );
                double idf =  ((e.getRank() *e.getCount()) /((double) t *100) );
...
*/
    ...
    public static double calculateIdf(long totalDocs, int numDocsContainingWord) {
        return Math.log(((totalDocs * 1.0) / numDocsContainingWord));
    }


    public static double getTf(int cnt, int docLen) {

        return (cnt / (float) docLen);
    }
